        ...
        for model in experiment.keys():
            if model not in list(combined_experiment_dict.keys()):
                ...
                for layer in combined_experiment_dict[model].keys():
                    combined_experiment_dict[model][layer] = combined_experiment_dict[model][layer] + experiment[model][
                        layer]
...
def plot_roi_analysis_for_models(models_to_plot,
    ...
    for model_idx, model_name in enumerate(models_to_plot):
        ...
        if analysis_type == 'regression':
            all_r2, all_alphas, all_voxel_info, all_conv_layers = load_all_voxel_predictions(all_networks_and_layers,
                                                                                             model_name)
            if not all_voxel_info:
                ...
                all_model_results_all_layers.append(
                    [np.ones(len(all_participants), len(all_conv_layers)) * np.nan for roi_idx in
                     range(len(roi_names))])
                continue
        ...
        save_best_layer_names[model_name] = {roi_name: median_best_layer_name[roi_idx] for roi_idx, roi_name in
                                             enumerate(roi_names)}

        average_metamer_recognition = [choose_metamer_layer(combined_experiment_dict[model_name], m, 'average') for m in
                                       median_best_layer_name]
        sem_metamer_recognition = [choose_metamer_layer(combined_experiment_dict[model_name], m, 'sem') for m in
                                   median_best_layer_name]
        ...
        average_all_metamer_recognition = [choose_metamer_layer(combined_experiment_dict[model_name], m, 'average') for
                                           m in all_conv_layers]
        sem_all_metamer_recognition = [choose_metamer_layer(combined_experiment_dict[model_name], m, 'sem') for m in
                                       all_conv_layers]
    ...
    for roi_plot_idx, roi_name in enumerate(roi_names):
        ...
        for model_idx, model_name in enumerate(models_to_plot):
            ...
            score_sem = (np.array(all_model_results)[model_idx, roi_plot_idx, :].std(axis=0)) / np.sqrt(
                num_participants)
