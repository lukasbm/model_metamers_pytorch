...
def display_all_visual_models_for_experiment(exp_name,
    ...
    experiment_name = all_model_info.TURK_IMAGE_EXPERIMENTS_GROUPINGS['experiment_%d' % exp_num][
    ...
    experiment_folder_web = all_model_info.TURK_IMAGE_EXPERIMENTS_GROUPINGS['experiment_%d' % exp_num][
        'experiment_folder_web']
    ...
    for model_idx, model in enumerate(plot_model_order):
        ...
        for plot_idx, layer in enumerate(plot_layers):
            try:
                ...
                ax = plt.subplot(1 + len(plot_model_order), max_num_plots,
                                 1 + (max_num_plots * (model_idx + 1)) + plot_idx)
                if 'hmax' in model:
                    plt.imshow(plt.imread(metamer_png_path), interpolation='none', cmap='Greys_r')
                else:
                ...
                print(
                    'Missing %s, this image was not included in this experiment. Try another example.' % orig_html_path)
    plt.show()
...
def display_all_audio_models_for_experiment(exp_name,
    ...
    experiment_name = all_model_info.TURK_AUDIO_EXPERIMENTS_GROUPINGS['experiment_%d' % exp_num][
        'paper_experiment_name']
    jsin_configs = all_model_info.TURK_AUDIO_EXPERIMENTS_GROUPINGS['experiment_%d' % exp_num]['experiment_params_web']

    experiment_folder_web = all_model_info.TURK_AUDIO_EXPERIMENTS_GROUPINGS['experiment_%d' % exp_num][
        'experiment_folder_web']
    ...
    for model_idx, model in enumerate(plot_model_order):
        ...
        for plot_idx, layer in enumerate(plot_layers):
            ...
            try:
                ...
                load_audio = False
                print(
                    'Missing %s, this audio was not included in this experiment. Try another example.' % metamer_html_path)
