...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, last_block=False):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, fake_relu=False):
        identity = x
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, with_latent=False, no_relu=False, fake_relu=False):
        if with_latent:
            all_outputs = {'input_after_preproc': x}
        x = x.type(self.conv1.weight.dtype)
        ...
        if with_latent:
            all_outputs['positional_embedding'] = x
        x = self.ln_pre(x)
        if with_latent:
            all_outputs['ln_pre'] = x

        x = x.permute(1, 0, 2)  # NLD -> LND
        if with_latent:
            x, _, all_outputs = self.transformer(x, with_latent=with_latent, all_outputs=all_outputs)
        else:
