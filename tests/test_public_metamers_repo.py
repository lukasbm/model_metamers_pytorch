...
VISION_NETWORK_LIST = ("alexnet", "cornet_s", "resnet50", "resnet101", "vgg_19",
                       ...
                       "alexnet_early_checkpoint", "alexnet_reduced_aliasing_early_checkpoint",
                       "vonealexnet_gaussian_noise_std4_fixed",
                       "texture_shape_resnet50_trained_on_SIN", "texture_shape_alexnet_trained_on_SIN",
                       "CLIP_resnet50", "CLIP_ViT-B_32", "SWSL_resnet50", "SWSL_resnext101_32x8d",
                       "vision_transformer_vit_large_patch16_224",
...
AUDIO_NETWORK_LIST = ("cochresnet50", "kell2018",
                      "cochresnet50_l2_1_robust_waveform", "cochresnet50_l2_p5_robust_waveform",
                      "cochresnet50_linf_p002_robust_waveform",
                      ...
                      "cochresnet50_l2_1_random_step_cochleagram", "cochresnet50_l2_1_random_step_waveform",
                      "cochresnet50_linf_p002_random_step_waveform",
                      "kell2018_l2_1_random_step_cochleagram", "kell2018_l2_1_random_step_waveform",
                      "kell2018_linf_p002_random_step_waveform")
...
class VisionNetworkTests(unittest.TestCase):
        ...
        for model in VISION_NETWORK_LIST:
            with self.subTest(model=model):
                build_network_spec = importlib.util.spec_from_file_location("build_network",
                                                                            os.path.join(self.model_directory_base,
                                                                                         model, 'build_network.py'))
    ...
    def test_generate_metamers(self):
        for model in VISION_NETWORK_LIST[-1:]:
            with self.subTest(model=model):
                print(model)
                make_metamers_imagenet_16_category_val_400_only_save_metamer_layers.main(
                    ('0 -I 2 -N 2 -D model_analysis_folders/visual_networks/%s -O' % model).split())

    def test_make_null(self):
        for model in VISION_NETWORK_LIST[-1:]:
            with self.subTest(model=model):
                print(model)
                make_null_distributions.main(
                    ('-N 20 -I 0 -R 0 --shuffle -D model_analysis_folders/visual_networks/%s -O True' % model).split())
    ...
    def test_generate_metamers(self):
        for model in AUDIO_NETWORK_LIST[-1:]:
            with self.subTest(model=model):
                ...
                make_metamers_wsj400_behavior_only_save_metamer_layers.main(
                    ('0 -I 2 -N 2 -D model_analysis_folders/audio_networks/%s -O' % model).split())
                pass
    ...
    def test_make_null(self):
        for model in AUDIO_NETWORK_LIST[-1:]:
            with self.subTest(model=model):
                ...
                # pass
                make_null_distributions.main((
                                                         '-N 20 -I 0 -R 0 --no-shuffle -D model_analysis_folders/audio_networks/%s -O True' % model).split())
