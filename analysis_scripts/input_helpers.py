...
def generate_import_audio_functions(audio_func='psychophysicskell2018dry', preproc_scaled=1, rms_normalize=1, **kwargs):
    ...
    if audio_func == 'psychophysicskell2018dry_overlap_jsinv3':
        return partial(psychophysicskell2018dry_overlap_jsinv3, preproc_scaled=preproc_scaled,
                       rms_normalize=rms_normalize, **kwargs)
    elif audio_func == 'psychophysics_wsj400_jsintest':
        return partial(psychophysics_wsj400_jsintest, preproc_scaled=preproc_scaled, rms_normalize=rms_normalize,
                       **kwargs)
    elif audio_func == 'load_specified_audio_path':
        return partial(use_audio_path_specified_audio, preproc_scaled=preproc_scaled, rms_normalize=rms_normalize,
                       **kwargs)
...
def psychophysicskell2018dry_overlap_jsinv3(WAV_IDX, preproc_scaled=1, rms_normalize=None, SR=20000):
    ...
    save_dry_path = os.path.join(ASSETS_PATH,
                                 'behavioralKellDataset_sr20000_kellBehavioralDataset_jsinv3overlap_dry_only.pckl')
...
def generate_import_image_functions(image_func='small_16_class_imagenet',  # default when gen: 400_16_class_imagenet_val
    ...
    if image_func == '256_16_class_imagenet':
        ...
        return partial(use_image_path_specified_image, image_path=image_path, image_class=image_class,
                       im_shape=im_shape, data_format=data_format)
...
def make_400_16_class_imagenet_val_data():
    ...
    image_classes = ['airplane', 'bear', 'bicycle', 'bird', 'boat', 'bottle', 'car', 'cat', 'chair', 'clock', 'dog',
    ...
    for image_class_idx, image_class in enumerate(image_classes):
        ...
        with open(os.path.join(output_image_path, 'image_list_full_400_16_class_imagenet_val_images.json'),
                  "w") as outfile:
...
def make_256_16_class_imagenet():
    ...
    for image_class_idx, image_class in enumerate(image_classes):
        ...
        random_images_perm = np.random.permutation(range(len(all_image)))[
                             10:26]  # grab 16 of each class, so they do not overlap with a precious set.
...
def get_multiple_samples_pytorch_datasets(NUM_EXAMPLES, DATA, train_or_val='val', im_shape=224, data_format='NHWC',
                                          START_IDX=0):
    print('Using pytorch dataset %s' % DATA)
    if im_shape != 224:
        raise NotImplementedError('Pytorch datasets not implemented for arbitrary shapes yet')
