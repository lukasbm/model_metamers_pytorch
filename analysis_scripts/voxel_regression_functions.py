...
def ridgeRegressSplits(features,
    ...
    ridgeCrossVal = sklearn.linear_model.RidgeCV(alphas=possible_alphas, normalize=ridge_normalize,
...
def runRidgeWithCorrectedR2_ThreeRunSplit(features,
    ...
    rv = max(3 * rv_med / (1 + 2 * rv_med),
             0.182)  # Value changed to 0.182 which corresponds to p<0.05 for correlations of two random gaussian variables of length 83 (length of the training data)
    rv_hat_med = np.nanmedian(rv_hats)
    rv_hat = max(3 * rv_hat_med / (1 + 2 * rv_hat_med),
                 0.183)  # Value changed to 0.183 which corresponds to p<0.05 for correlations of two random gaussian variables of length 82 (length of the test data)
