...
def _get_activations(all_outputs, metamer_layers):
    ...
    for layer in metamer_layers:
        if isinstance(all_outputs[layer], list):
            activations_1.append(np.concatenate(
                [out_value[0, :].detach().cpu().numpy().ravel().astype(np.float32) for out_value in all_outputs[layer]],
                0))
...
def run_null_distribution(NUMNULL, SPLITIDX, PATHNULL, RANDOMSEED, OVERWRITE_PICKLE, shuffle, MODEL_DIRECTORY):
    ...
    pckl_path = os.path.join(MODEL_DIRECTORY, PATHNULL,
                             'null_dist_N_%d_RS_%d_SPLIT_%d_START_%d.pckl' % (NUMNULL, RANDOMSEED, SPLITIDX, STARTIDX))
    ...
    train_loader, val_loader = ds.make_loaders(batch_size=BATCH_SIZE,
                                               ...
                                               # Need to shuffle to get different null samples each time.
                                               shuffle_val=shuffle,
                                               data_aug=True,
                                               )
    ...
    for null_iter in range(NUMNULL):
        ...
        for layer_idx, layer in enumerate(metamer_layers):
            if (np.sum(activations_1[layer_idx].ravel()) == 0) or (np.sum(activations_2[layer_idx].ravel()) == 0):
                ...
                db_SNR_i, norm_signal_i, norm_noise_i = compute_snr_db(
                    [activations_1[layer_idx], activations_2[layer_idx]])
...
def main(raw_args=None):
    ...
    parser.add_argument('-N', '--NUMNULL', metavar='--N', type=int, default=1000000,
                        help='Number of samples to generate for the null distribution')
    parser.add_argument('-P', '--PATHNULL', metavar='--P', type=str, default='null_dist',
                        help='Path to store the null distribution output. If a relative path, creates in this directory')
    parser.add_argument('-I', '--SPLITIDX', metavar='--I', type=int, default=0,
                        help='Starting location for the null distribution reading will be (2*NUM_NULL)*SPLITIDX, so that there is no overlap between the splits. Note: for faster file reading, it can be helpful to only use even or odd splits')
    parser.add_argument('-R', '--RANDOMSEED', metavar='--R', type=int, default=0,
                        help='random seed to use for synthesis')
    parser.add_argument('-O', '--OVERWRITE_PICKLE', metavar='--P', type=bool, default=False,
                        help='set to true to overwrite the saved pckl file, if false then exits out if the file already exists')
    parser.add_argument('--shuffle', dest='shuffle', action='store_true',
                        help='Shuffle the input loaders (both train and val). Default is to shuffle the loaders.')
    parser.add_argument('--no-shuffle', dest='shuffle', action='store_false',
                        help='Turn off shuffling for input loaders (both train and val)')
