...
parser.add_argument('-N', '--NUMSAMPLES', metavar='--N', type=int, default=16,
                    help='number of samples to evaluate from the test data')
parser.add_argument('-I', '--MAXITER', metavar='--I', type=int, default=32,
                    help='maximum number of iterations to generate adversarial examples')
parser.add_argument('-B', '--BATCHSIZE', metavar='--B', type=int, default=16,
                    help='batch size for evaluation, set based on GPU limitations')
parser.add_argument('-T', '--ATTACK_TYPE', metavar='--T', type=str, default='inf',
                    help='norm of the adversarial perturbation (“inf”, 1 or 2.)')
parser.add_argument('-D', '--EPS_DIVIDE', metavar='--D', type=int, default=5,
                    help='divide the eps by this to set the step size')
...
parser.add_argument('-U', '--NUM_EPS_STEPS_BETWEEN', metavar='--U', type=int, default=2,
                    help='number of eps steps between each power of 10')
...
parser.add_argument('-E', '--ATTACK_EPS_LIST', metavar='--E', type=str,
                    help='if given, use this as the list of attacks rather than the range between min and max eps. Separate by commas.')
